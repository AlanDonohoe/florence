(function($) {
    $.fn.slideshowify = function() {
        var _self = this, _imgs = [], _imgIndex =- 1, _imgIndexNext = 0, _transition = true, _easing = 'in-out', _viewEl = document, _containerId = "slideshowify-" + new Date().getTime(), _containerCSS = {
            "position": "absolute",
            "overflow": "hidden",
            "z-index": "-2",
            "left": "0",
        ...
        function _revealImg(curImg) {
            var $img = $(this), viewW = _$viewEl.width(), viewH = _$viewEl.height(), viewRatio = viewW / viewH, imgRatio = $img.width() / $img.height(), marginThreshold = Math.floor(Math.max(viewW, viewH) / 10), direction = Math.round(Math.random()), transAttr = {}, transProps, marginPixels, modDims;
        ...
        function _loadImg() {
            var img = new Image(), nextImg = new Image(), len = _imgs.length;
        ...
        $("<div id='" + _containerId + "'></div>").css(_containerCSS).appendTo(_cfg.parentEl);
        _loadImg();
        return this;
    };
